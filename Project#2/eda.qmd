...
ggplot(planetdata_cleaner, aes(x=discoverymethod))+geom_bar()+theme(axis.text = element_text(size=3)) #makes a graph comparing how often a discovery method is used
```
...
filtered_data <- planetdata_cleaner %>%
  ...
  select(discoverymethod, pl_orbper, pl_massj, pl_orbsmax) %>% #filters the data so only these columns are present
