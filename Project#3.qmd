...
K-means clustering is the analysis that is currently being used as it serves to group together variables to find any patterns that might be present. In the code below, only two variables are being used to test k-means clustering. The final version will include a multitude of variables.
...
planetdata_final <- subset(planetdata_cleaner, discoverymethod %in% c("Transit", "Radial Velocity", "Micronlensing", "Imaging"))
...
planetdata_final[,c("pl_orbper", "pl_orbsmax")] <- scale(planetdata_final[,c("pl_orbper", "pl_orbsmax")])
kmeanscols <- planetdata_final[,c("pl_orbper", "pl_orbsmax")]
set.seed(123)
km.out <- kmeans(kmeanscols, centers = 3, nstart = 20)
km.out
planetdata_final$cluster_id <- factor(km.out$cluster)
ggplot(planetdata_final, aes(x = pl_orbper, y = pl_orbsmax, color = cluster_id, shape = discoverymethod)) +
